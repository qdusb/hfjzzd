...
<div id="mainDiv">
<ul>
<li class="explode">网站配置</li>
	<ul>
 	<?php
		$sql="select id,default_file,name from advanced where state>0 and tag='网站配置' order by shownum asc";
		$query=$db->query($sql);
...
<li class="explode">隐藏分类管理</li>
	<ul>
 	<?php
		$sql="select id,default_file,name from advanced where state>0 and tag='隐藏分类管理' order by shownum asc";
		$query=$db->query($sql);
		while($arr=mysql_fetch_array($query)){
